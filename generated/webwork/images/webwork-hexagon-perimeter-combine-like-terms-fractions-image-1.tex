...
\begin{tikzpicture}
  \draw[fill=wood] (0,0)
  -- node[pos=0.5,below]{\(\left({\frac{1}{3}}\right)W\)} (1,0)
  -- node[pos=0.5,below right]{\(W\)} (3,1)
  -- node[pos=0.5,above right]{\(\left({\frac{10}{9}}\right)L\)} (2,3)
  -- node[pos=0.5,above left]{\(2L\)} (-1,3)
  -- node[pos=0.5,left]{\(\left({\frac{8}{9}}\right)H\)} (-1,1)
  -- node[pos=0.5,below left]{\(\left({\frac{2}{3}}\right)H\)} cycle;   
\end{tikzpicture}
