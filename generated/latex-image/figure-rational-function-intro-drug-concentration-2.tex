...
\begin{tikzpicture}
    \begin{axis}[
                ymin=-0.25,ymax=1,xmin=-1,xmax=12,
                ytick={0,0.25,0.5,0.75},
                xtick={0,1,...,11},
                xlabel={$t$, time in hours},ylabel={$y$, concentration (mg per liter)}]
        \addplot+[domain=-0.708:12,samples=100] {3*x/(x^2+8)} node[below left] {$y=C(t)$};
        \addplot+[domain=-1:12] {0.35} node[pos=0.23, above right] {$y=0.35$};
        \addplot+[soliddot, color=sixthcolor] coordinates {(1.066,0.35)} node [below right] {(1.066,0.35)};
        \addplot+[soliddot, color=sixthcolor] coordinates {(7.506,0.35)} node [above right] {(7.506,0.35)};
        \addplot+[soliddot, color=sixthcolor] coordinates {(2.828,0.53)} node [above] {(2.828,0.53)};
    \end{axis}
\end{tikzpicture}%
}
\end{document}
