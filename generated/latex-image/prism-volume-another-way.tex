...
\begin{tikzpicture}%(x,z,y)
  %\node[white] at (2,0,0) {.};
  \draw[fill=wood] (0,0,0) -- ++(0,-2,0) -- ++(-4,0,0) -- ++(0,2,0) node[pos=0.5,above,sloped] {$2\,\text{cm}$} -- cycle; %front face
  \draw[fill=wood!20] (0,0,0) -- ++(0,0,-3) -- ++(0,-2,0) -- ++(0,0,3) -- cycle; %side
  \draw[fill=wood!20] (0,0,0) -- ++(-4,0,0) -- ++(0,0,-3) node[pos=0.5,above,sloped] {$3\,\text{cm}$} -- ++(4,0,0) node[pos=0.5,above] {$4\,\text{cm}$} -- cycle; %top
\end{tikzpicture}%
}
\end{document}
