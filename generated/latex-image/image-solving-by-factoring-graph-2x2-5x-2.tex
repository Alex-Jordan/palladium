...
\begin{tikzpicture}
    \begin{axis}[xmin=-5,
        ...
        \addplot+[domain=-3.162:3.162] {2*x^2} node[pos=0.3, left] {$y=2x^2$};
        \addplot+[domain=-1.2:4] {5*x} node[pos=0.05, left] {$y=5x$};
        \addplot[soliddot,color=fifthcolor] coordinates {(0,0)} node[below right] {$(0,0)$};
        \addplot[soliddot,color=fifthcolor] coordinates {(2.5,12.5)} node[right] {$(\frac{5}{2},12.5)$};
    \end{axis}
\end{tikzpicture}%
}
\end{document}
