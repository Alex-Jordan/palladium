...
\begin{tikzpicture}
    \begin{axis}[
                ymin=-0.25,ymax=1,xmin=-10,xmax=120,
                ytick={0,0.25,0.5,0.75},
                xtick={0,24,...,96},
                minor xtick={0,12,...,120},
                xlabel={$t$, time in hours},ylabel={concentration (mg per liter)}]
        \addplot+[domain=-0.708:120,samples=200] {3*x/(x^2+8)} node[pos=0.1, above right] {$y=C(t)$};
    \end{axis}
\end{tikzpicture}%
}
\end{document}
